...
PACOTES_REMOVIVEIS=(
    # Ecossistema Snap (cuidado: Firefox pode quebrar)
    "snapd" "snap-confine" "gnome-software-plugin-snap"
    
    # Jogos
    "gnome-mahjongg" "gnome-mines" "gnome-sudoku" "aisleriot" "gnome-chess"
    
    # Aplicativos de desktop desnecessários
    "thunderbird" "transmission-common" "rhythmbox" "simple-scan"
    "cheese" "gnome-user-docs" "yelp" "example-content" "totem"
    
    # Suporte a hardware antigo
    "printer-driver-*" "hplip" "libreoffice-*"
    
    # Pacotes de idioma (mantenha apenas o seu)
    "ibus*" "fcitx*" "im-config" "language-pack-gnome-*"
    
    # Relatórios e telemetria
    "popularity-contest" "apport" "whoopsie" "ubuntu-report"
    
    # Acessibilidade (se não usar)
...
echo -e "${AZUL}Limpando pacotes órfãos...${NC}"
apt autoremove --purge -y

# Limpar cache
echo -e "${AZUL}Limpando cache de pacotes...${NC}"
apt clean
apt autoclean
...
PACOTES_ESSENCIAIS=(
    ...
    "pipx" "jq" "yq" "htop" "bashtop"
    
    # Utilitários úteis
    "tmux" "screen" "zip" "unrar" "testdisk" "fdupes"
)
...
# Atualizar sistema
echo -e "${AZUL}Executando atualização final do sistema...${NC}"
apt update
apt upgrade -y

# Criar lista de pacotes removidos para referência
echo -e "${AZUL}Criando relatório de remoção...${NC}"
